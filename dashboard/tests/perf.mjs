...
const result = {
  scenario_fingerprint: {
    host: "macbook m4 pro (arm64, darwin 25.2.0)",
...
if (mode !== "--node-only") {
  ...
  result.S1_browser = {
    ...
    unlock_to_kpis_ms: {
      p50: pct(u, 50),
      p95: pct(u, 95),
      note: "includes 600k PBKDF2 deriveKey (webcrypto, security parameter)",
    },
  };
}
...
if (mode !== "--browser-only") {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    execFileSync("node", ["tools/fleet-llm.mjs", "--format", "json"], {
      cwd: root,
      stdio: "ignore",
      env: { ...process.env, ...env },
    });
  ...
  try {
    execFileSync(
      "node",
      ["--cpu-prof", "--cpu-prof-dir", outDir, "tools/fleet-llm.mjs", "--format", "json"],
...
result.S3_dist_bytes = Object.fromEntries(
  assets.map((a) => [a, statSync(join(root, "dist/assets", a)).size]),
);
