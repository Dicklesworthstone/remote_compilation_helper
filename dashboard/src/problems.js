...
export const PROBLEM_KINDS = {
  ...
  "fleet.degraded": {
    severity: "warn",
    meaning:
      "Two or more dev machines report partial remote capability for the SAME worker-side reason. One root cause, not N machine problems — fix the listed workers.",
    fix: "Act on the worker.* rows; the dev machines recover on their own.",
  },
  "dev.unreachable": {
    severity: "critical",
    meaning:
      "The collector could not get `rch status` from this dev machine (ssh failed, rch missing, or rchd down). Its builds may be running locally with nobody watching.",
    fix: "`ssh <id> true` if the transport failed; otherwise `rch daemon start` on the box.",
  },
  "dev.local-only": {
    severity: "critical",
    meaning:
      "This dev machine is compiling on itself: posture local_only, or under half of its recent builds went to the pool.",
    fix: "`rch doctor` on the box, then act on what it names.",
  },
  "dev.degraded": {
    severity: "warn",
    meaning:
      "This dev machine sees partial remote capability (some workers unavailable or pressure-blocked) for a reason not shared fleet-wide.",
    fix: "Read its remediation hints (`rch status --json` on the box).",
  },
  "dev.hook_missing": {
    severity: "critical",
    meaning:
      "Claude Code's PreToolUse hook is NOT installed on this dev machine. Critical when no working cargo shim is installed either (nothing is intercepted); warn when the shim still covers cargo (only bun/gcc/make/nix builds from agents run locally).",
    fix: "`rch hook install` on the box.",
  },
  "dev.shim_missing": {
    severity: "warn",
    meaning:
      "The cargo shim is not installed, so builds started by scripts/Makefiles (not via a hooked tool call) run locally.",
  ...
  "dev.unmanaged_local_builds": {
    severity: "critical",
    meaning:
      "Compiler processes are running on this dev machine RIGHT NOW with no rch ancestor — builds the hook/shim never saw. This is the 'silently burning local cores' failure. Linux dev machines only: rch's detector reads /proc, so a macOS box always reports 0 and this kind never fires there.",
  ...
  "dev.doctor_failed": {
    severity: "critical",
    meaning:
      "`rch doctor` reports FAILED checks on this dev machine (hook, daemon socket, config, SSH keys...).",
  ...
  "dev.collection_error": {
    severity: "warn",
    meaning:
      "The box answered `rch status` but one of the other probes failed (metrics endpoint, workers list, doctor, shim, hook). Some columns for this machine are UNKNOWN, not fine.",
    fix: "`rch doctor` on the box; the detail names the probe that failed.",
  },
  "worker.offline": {
    severity: "critical",
    meaning:
      "Worker unreachable, circuit breaker open, or not seen for over an hour. Its slots are gone from every dev machine.",
    fix: "`rch workers probe <id>` from a dev machine; the action column carries rch's own advice when it has one.",
  },
  "worker.critical": {
    severity: "critical",
    meaning:
      "Critical pressure (disk ≥95% or the daemon's pressure policy fired). Admission is refused; every dev machine derates it to 0 slots.",
    fix: "Free disk on the worker (`rch gc --workers <id>`, `rch cache clean --workers <id>`), then `rch workers capabilities --refresh`.",
  },
  "worker.warn": {
    severity: "warn",
    meaning:
      "Pressure warning, disk ≥88%, load ≥2× cores, recent failures, half-open circuit, drained, or stale telemetry.",
    fix: "See the action column; usually disk reclaim or wait for the next telemetry poll.",
  },
  "worker.convergence_drift": {
    severity: "warn",
    meaning:
      "This worker is missing repositories that a dev machine's builds depend on (repo convergence not ready).",
    fix: "`rch doctor --reliability` on the dev machine that reported it.",
  },
  "build.hook_dead": {
    severity: "critical",
    meaning:
      "An active build's dispatching hook process is gone. The build cannot complete and its slots stay reserved until cancelled.",
    fix: "`rch cancel <build id>` on the dev machine that owns it.",
  },
  "build.stalled": {
    severity: "warn",
    meaning:
      "An active build has stopped heartbeating and progressing, or has made no progress for 30+ minutes with the daemon's stall detector fairly confident.",
...
function isTransportError(msg) {
  return /(^|[\s:])ssh[:\s]|command failed: ssh|route to host|connection refused|connection reset|connection timed out|timed out|permission denied|host key|killed by/i.test(
    msg,
  );
}
...
export function fleetVersion(devs) {
  ...
  return {
    version: modal,
    machines: best < 0 ? 0 : [...counts.values()].reduce((a, b) => a + b, 0),
    off_version: off,
  };
}
...
export function buildProblems({
  ...
  for (const d of reachable) {
    for (const h of d.remediation_hints ?? []) {
      ...
      if (!prev || rank < prev.rank)
        adviceFor.set(h.worker_id, { rank, action: h.suggested_action, on: d.id });
    }
  }
  ...
  const rootCauseWorkers = workers.filter(
    (w) =>
      w.health === "critical" ||
      w.health === "offline" ||
  ...
  for (const w of workers) {
    ...
    if (w.recovery_in_secs != null && w.recovery_in_secs > 0)
      detail += ` — circuit retries in ${fmtSecs(w.recovery_in_secs)}`;
    ...
    problems.push(
      row(w.health === "warn" ? "warn" : "critical", `worker.${w.health}`, w.id, detail, {
  ...
  const sameReason =
    degraded.length >= 2 && degraded.every((d) => dReason(d) === dReason(degraded[0]));
  ...
  for (const d of devs) {
    ...
    if (d.level === "unreachable") {
      ...
      problems.push(
        row("critical", "dev.unreachable", d.id, dReason(d), {
    ...
    if (d.level === "local-only") {
      problems.push(
        row("critical", "dev.local-only", d.id, dReason(d), { action: "rch doctor", on: d.id }),
      );
    } else if (d.level === "degraded" && !collapseDegraded) {
      problems.push(
        row("warn", "dev.degraded", d.id, dReason(d), { action: "rch status --json", on: d.id }),
      );
    }
    ...
    if (d.shim) {
      if (d.shim.installed === false) {
        ...
        const why =
          d.shim.up_to_date === false ? "out of date" : "shadowed by another cargo on PATH";
        problems.push(
          row("warn", "dev.shim_stale", d.id, `cargo shim ${why}`, {
            action: "rch shim install",
            on: d.id,
          }),
        );
      }
      ...
      if (isHookDead(b)) {
        problems.push(
          row(
            ...
            {
              since: b.started_at,
              action: b.id ? `rch cancel ${b.id}` : "rch queue --json",
              on: d.id,
            },
          ),
        );
  ...
  for (const p of problems) {
    ...
    if (prev) {
      ...
      if ((SEVERITY_RANK[p.severity] ?? 9) < (SEVERITY_RANK[prev.severity] ?? 9))
