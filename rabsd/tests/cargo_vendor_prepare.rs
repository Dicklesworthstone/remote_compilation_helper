...
fn read_log(path: &Path) -> Vec<u8> {
    ...
    assert!(bytes.len() as u64 <= LOG_LIMIT, "fixture output exceeded its bound");
    bytes
}
...
impl Process {
    fn spawn(owner: &Path, name: &str, command: &mut Command) -> Self {
        ...
        let child = command.process_group(0).stdin(Stdio::null())
            .stdout(File::create(&stdout).unwrap()).stderr(File::create(&stderr).unwrap())
            .spawn().expect("fixture executable must be available; missing tools are not a pass");
        Self { child, stdout, stderr, reaped:false }
    }
    fn wait(&mut self, budget: Duration) -> (ExitStatus, Vec<u8>, Vec<u8>) {
        let until = Instant::now() + budget;
        loop {
            for path in [&self.stdout, &self.stderr] {
                assert!(fs::metadata(path).unwrap().len() <= LOG_LIMIT, "unbounded fixture output");
            }
            if let Some(status) = self.child.try_wait().unwrap() {
                self.reaped = true;
                return (status, read_log(&self.stdout), read_log(&self.stderr));
            }
            assert!(Instant::now() < until, "fixture timeout: {}", String::from_utf8_lossy(&read_log(&self.stderr)));
    ...
    fn listening(&mut self) -> String {
        ...
        loop {
            ...
            if let Some(status) = self.child.try_wait().unwrap() {
                self.reaped = true;
                panic!("receiver exited before listening ({status}): {}", String::from_utf8_lossy(&log));
            }
...
impl Drop for Process {
    fn drop(&mut self) {
        if self.reaped || self.child.try_wait().ok().flatten().is_some() { return; }
        let group = format!("-{}", self.child.id());
        let signal = |name: &str| {
            let _ = Command::new("/bin/kill").args([name, "--", group.as_str()])
...
fn command(program: impl AsRef<std::ffi::OsStr>) -> Command {
    ...
    command.env_clear();
    for name in ["PATH", "LD_LIBRARY_PATH", "DYLD_LIBRARY_PATH", "RUSTUP_HOME", "RUSTUP_TOOLCHAIN"] {
...
impl Fixture {
    fn new() -> Self {
        ...
        assert!(rustc.is_file(), "run this fixture with a complete installed toolchain, not only a rustup proxy");
        write(&anchor, "Cargo.toml", "[workspace]\nmembers=['app','local_dep']\nresolver='2'\n");
        write(&anchor, ".cargo/config.toml", "[source.crates-io]\nreplace-with='vendored-sources'\n[source.vendored-sources]\ndirectory='vendor'\n");
        write(&anchor, "app/Cargo.toml", "[package]\nname='vendor_app'\nversion='0.1.0'\nedition='2021'\n[dependencies]\nlocal_dep={path='../local_dep'}\nregistry_dep='=1.0.0'\n");
        write(&anchor, "app/src/main.rs", "fn main() { println!(\"{}:{}\", registry_dep::answer()+local_dep::answer(), env!(\"BUILD_LABEL\")); }\n");
        write(&anchor, "local_dep/Cargo.toml", "[package]\nname='local_dep'\nversion='0.1.0'\nedition='2021'\n");
        write(&anchor, "local_dep/src/lib.rs", "pub fn answer() -> u32 { 2 }\n");
        let leaf = Self::vendor(&anchor, "registry_leaf", BTreeMap::from([
            ("Cargo.toml", "[package]\nname='registry_leaf'\nversion='1.0.0'\nedition='2021'\n"),
            ("src/lib.rs", "pub fn answer() -> u32 { 40 }\n"),
        ]));
        let dep = Self::vendor(&anchor, "registry_dep", BTreeMap::from([
            ("Cargo.toml", "[package]\nname='registry_dep'\nversion='1.0.0'\nedition='2021'\nbuild='build.rs'\n[dependencies]\nregistry_leaf='=1.0.0'\n[build-dependencies]\nregistry_leaf='=1.0.0'\n"),
            ("build.rs", "fn main() { let n: u32 = std::fs::read_to_string(\"payload.txt\").unwrap().trim().parse().unwrap(); assert_eq!(n, registry_leaf::answer()); let out=std::path::PathBuf::from(std::env::var_os(\"OUT_DIR\").unwrap()); std::fs::write(out.join(\"generated.rs\"), format!(\"pub const GENERATED:u32={n};\")).unwrap(); println!(\"cargo:rerun-if-changed=payload.txt\"); }\n"),
            ("src/lib.rs", "include!(concat!(env!(\"OUT_DIR\"),\"/generated.rs\")); pub fn answer() -> u32 { GENERATED+registry_leaf::answer()-40 }\n"),
            ("payload.txt", "40\n"),
            ...
            ("test-fixtures/Cargo.toml", "not a Cargo manifest; ordinary fixture data\n"),
        ]));
        write(&anchor, "Cargo.lock", format!("version=4\n\n[[package]]\nname='local_dep'\nversion='0.1.0'\n\n[[package]]\nname='registry_dep'\nversion='1.0.0'\nsource='{REGISTRY}'\nchecksum='{dep}'\ndependencies=['registry_leaf']\n\n[[package]]\nname='registry_leaf'\nversion='1.0.0'\nsource='{REGISTRY}'\nchecksum='{leaf}'\n\n[[package]]\nname='vendor_app'\nversion='0.1.0'\ndependencies=['local_dep','registry_dep']\n"));
        ...
        Self { owner, anchor, cargo, rustc, specification }
    }
    fn vendor(root: &Path, name: &str, files: BTreeMap<&str, &str>) -> String {
        ...
        write(root, &format!("vendor/{name}/.cargo-checksum.json"),
    ...
    fn prepare(&self, name: &str) -> (PathBuf, ExitStatus, Vec<u8>, Vec<u8>) {
        ...
        cmd.arg("--worker-prepare").arg(&self.anchor).arg(spec).arg(&bundle)
            .env("CARGO", &self.cargo).env("RUSTC", &self.rustc)
            .env("RUSTC_WRAPPER", "/must-not-execute-a-wrapper");
        let result = Process::spawn(self.owner.path(), name, &mut cmd).wait(Duration::from_secs(45));
    ...
    fn prepared(&self) -> (PathBuf, Value) {
        ...
        let request: Value = serde_json::from_slice(&fs::read(bundle.join("request.json")).unwrap()).unwrap();
        ...
        assert_eq!(request, expected, "only preparation intent becomes a manifest");
        assert_eq!(fs::read(bundle.join("source/.cargo/config.toml")).unwrap(), fs::read(self.anchor.join(".cargo/config.toml")).unwrap());
        assert_eq!(fs::read(bundle.join("source/Cargo.lock")).unwrap(), fs::read(self.anchor.join("Cargo.lock")).unwrap());
        (bundle, request)
    }
}
...
fn real_preparation_and_frozen_cargo_build_use_captured_transitive_and_build_script_inputs() {
    ...
    write(&fixture.anchor, "vendor/registry_leaf/src/lib.rs", "pub fn answer() -> u32 { 99 }\n");
    fs::rename(&fixture.anchor, fixture.owner.path().join("retired-checkout")).unwrap();
    let home = fixture.owner.path().join("local-cargo-home"); fs::create_dir(&home).unwrap();
    let target = fixture.owner.path().join("local-target");
    let mut cmd = command(&fixture.cargo);
    cmd.current_dir(bundle.join("source")).args(["build","--frozen","--jobs=1","-p","vendor_app"])
        .arg("--target-dir").arg(&target).env("CARGO_HOME", &home).env("RUSTC", &fixture.rustc)
        .env("CARGO_INCREMENTAL", "0").env("BUILD_LABEL", LABEL);
    let (status, _, stderr) = Process::spawn(fixture.owner.path(), "cargo-build", &mut cmd).wait(Duration::from_secs(90));
    assert!(status.success(), "{}", String::from_utf8_lossy(&stderr));
    let (status, stdout, stderr) = Process::spawn(fixture.owner.path(), "built-binary",
        &mut command(target.join("debug/vendor_app"))).wait(Duration::from_secs(5));
    assert!(status.success(), "{}", String::from_utf8_lossy(&stderr));
    assert_eq!(stdout, format!("42:{LABEL}\n").as_bytes());
    let image = capture_sealed_source(&[("workspace".into(), bundle.join("source"))], false, 2, 2_000_000).unwrap();
    assert_eq!(SourceUpload::for_request(Arc::new(image), "workspace", &request).unwrap().wire_manifest(), request["source_manifest"]);
    assert!(fs::read_dir(home).unwrap().all(|entry| entry.unwrap().file_name() != "credentials.toml"));
}
...
fn corrupted_or_unapproved_vendor_inputs_never_publish_a_prepared_bundle() {
    for case in 0..4 {
        ...
        match case {
            ...
            1 => write(&fixture.anchor, "vendor/registry_dep/unlisted.txt", "not approved by the checksum map"),
            2 => { fixture.specification["cargo_source"]["vendor"] = json!("../outside"); }
            _ => write(&fixture.anchor, ".cargo/config.toml", "[source.crates-io]\nreplace-with='vendor'\n[source.vendor]\ndirectory='/outside'\n"),
        }
...
fn lock_mismatch_cannot_be_blessed_by_matching_vendor_file_hashes() {
    ...
    assert!(stdout.is_empty());
    assert!(!bundle.exists());
    assert!(fs::read_to_string(path).unwrap().contains(&"00".repeat(32)), "planning may not repair the original lock");
}
...
impl Certificates {
    fn new(root: &Path) -> Self {
        let run = |args: &[&str]| {
            let mut cmd = command("openssl"); cmd.current_dir(root).args(args);
            let (status, _, errors) = Process::spawn(root, "openssl", &mut cmd).wait(Duration::from_secs(15));
            assert!(status.success(), "{}", String::from_utf8_lossy(&errors));
        };
        run(&["req","-x509","-newkey","rsa:2048","-nodes","-sha256","-days","1",
            "-subj","/CN=Vendor Test CA","-keyout","ca.key","-out","ca.pem",
        ...
        for (name, usage, serial) in [("server","serverAuth","2"),("worker","clientAuth","3")] {
            ...
            let cert = format!("{name}.pem"); let ext = format!("{name}.ext");
            ...
            run(&["x509","-req","-in",&csr,"-CA","ca.pem","-CAkey","ca.key","-set_serial",serial,"-days","1","-sha256","-extfile",&ext,"-out",&cert]);
        }
        let files = |name: &str| rabs_asupersync::worker_transport::TlsFiles {
            ca:root.join("ca.pem"),certificate:root.join(format!("{name}.pem")),private_key:root.join(format!("{name}.key")),
        };
        Self { server:files("server"),worker:files("worker") }
    }
}
...
fn automatic_vendored_bundle_builds_over_native_tls_and_replays_installed_outputs_offline() {
    ...
    assert!(missing.is_empty(), "canonical isolation is required: {missing:?}");
    let worker_binary = PathBuf::from(std::env::var_os("RABS_TEST_WORKER_BIN")
    ...
    let (bundle, request) = fixture.prepared();
    fs::rename(&fixture.anchor, fixture.owner.path().join("retired-checkout")).unwrap();
    let credentials = fixture.owner.path().join("credentials"); fs::create_dir(&credentials).unwrap();
    let certificates = Certificates::new(&credentials);
    let identity = certificates.worker.local_identity().unwrap();
    let pin: String = identity.fingerprint.iter().map(|byte| format!("{byte:02x}")).collect();
    ...
    receiver_command.args(args).arg(&bundle).arg(&delivery).arg(&installed)
        .env("RABS_STATE_DIR", fixture.owner.path().join("coordinator-state"))
    ...
    worker_command.args(["--coordinator", &address, "--worker-id", WORKER, "--once"])
    ...
    assert_eq!(report["delivery"]["receipt"]["transport_authenticated"], true);
    ...
    let (status, output, errors) = Process::spawn(fixture.owner.path(), "installed-binary",
        &mut command(installed.join("debug/vendor_app"))).wait(Duration::from_secs(5));
    assert!(status.success(), "{}", String::from_utf8_lossy(&errors));
    ...
    fs::rename(bundle.join("source"), fixture.owner.path().join("retired-source")).unwrap();
    let mut offline = command(env!("CARGO_BIN_EXE_rabsd"));
    offline.args(args).arg(&bundle).arg(&delivery).arg(&installed);
    let (status, output, errors) = Process::spawn(fixture.owner.path(), "offline-replay", &mut offline).wait(Duration::from_secs(15));
    assert!(status.success(), "{}", String::from_utf8_lossy(&errors));
    ...
    assert_eq!(repeated["delivery"]["receipt"], report["delivery"]["receipt"]);
    assert_eq!(repeated["reexecute"], false);
    assert_eq!(fs::read(delivery.join("delivery.json")).unwrap(), receipt);
    assert_eq!(serde_json::from_slice::<Value>(&fs::read(bundle.join("request.json")).unwrap()).unwrap(), request);
}
