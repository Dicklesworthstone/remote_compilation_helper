...
pub fn parse_archive_key(key: &str) -> Result<TypedDigest, String> {
    let body = key.strip_prefix(ATP_OBJECT_CONTENT_DOMAIN).and_then(|s| s.strip_prefix(':'))
        .ok_or_else(|| "archive key must be a native CAS content digest".to_owned())?;
    if body.len() != 64 || !body.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)) {
        return Err("archive key must contain 64 lowercase hex digits".to_owned());
    }
    let mut bytes = [0; 32];
    for (slot, pair) in bytes.iter_mut().zip(body.as_bytes().chunks_exact(2)) {
        let digit = |b: u8| if b <= b'9' { b - b'0' } else { b - b'a' + 10 };
        *slot = digit(pair[0]) * 16 + digit(pair[1]);
    }
    Ok(TypedDigest { algorithm: DigestAlgorithm::Sha256V1, domain: ATP_OBJECT_CONTENT_DOMAIN, bytes })
}
...
struct Item { len: u64, raw_sha256: String, executable: bool }
...
fn items(request: &Value, receipt: &Value) -> io::Result<BTreeMap<String, Item>> {
    ...
    for stream in ["stdout", "stderr"] {
        entries.insert(format!("diagnostics/{stream}"), Item {
    ...
    {
        let names: BTreeSet<&str> = declaration["files"].as_array()
            .ok_or_else(|| invalid("artifact declaration"))?.iter()
            .map(|value| value.as_str().ok_or_else(|| invalid("artifact name")))
            .collect::<io::Result<_>>()?;
        let rows = receipt["artifact_manifest"]["files"].as_array()
            .ok_or_else(|| invalid("artifact manifest"))?;
        require(rows.len() == names.len(), "artifact set mismatch")?;
        for (row, name) in rows.iter().zip(names) {
            require(text(row, "name")? == name, "artifact order or name mismatch")?;
            entries.insert(format!("artifacts/{name}"), Item {
                len: number(row, "bytes")?, raw_sha256: text(row, "sha256")?.to_owned(),
                executable: row.get("executable").and_then(Value::as_bool)
    ...
    for item in entries.values() {
        require(item.raw_sha256.len() == 64 && item.raw_sha256.bytes()
            .all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)), "invalid raw digest")?;
        total = total.checked_add(item.len).ok_or_else(|| invalid("delivery length overflow"))?;
        require(total <= MAX_DELIVERY_BYTES, "delivery budget exceeded")?;
    }
    require(total == number(receipt,"total_bytes")?, "delivery total mismatch")?;
    Ok(entries)
}
...
fn stream(reader: &mut dyn Read, sink: &mut dyn Write, limit: u64) -> io::Result<DigestSet> {
    let mut writer = StreamingObjectWriter::new(DigestRequest { blake3:false, raw_sha256:true }, None);
    let mut total = 0_u64;
    ...
    loop {
        ...
        if count == 0 { break; }
        total = total.checked_add(count as u64).ok_or_else(|| invalid("byte count overflow"))?;
...
fn matches_item(digests: &DigestSet, item: &Item) -> bool {
    digests.logical_size == item.len
        && digests.raw_sha256.is_some_and(|raw| hex(&raw.0) == item.raw_sha256)
}

fn ordinary_file(path: &Path) -> io::Result<File> {
    require(fs::symlink_metadata(path)?.is_file(), "CAS/delivery source is not an ordinary file")?;
    let file = File::open(path)?;
    require(file.metadata()?.is_file(), "CAS/delivery source changed type")?;
    Ok(file)
}
...
fn open_object(
    store: &mut dyn RabsMetadataStore, object: &TypedDigest, limit: u64,
    ...
    for (path, encoding, durable) in store.object_locations(object).map_err(failure)? {
        if encoding != RAW_PROFILE_V1 || !durable { continue; }
        let Ok(mut file) = ordinary_file(Path::new(&path)) else { continue; };
        if file.metadata()?.len() > limit { continue; }
        let digests = match stream(&mut file, &mut io::sink(), limit) {
            Ok(digests) => digests,
            Err(_) => continue,
        };
        if digests.atp_content_id != *object {
            store.set_location_quarantined(object, &path, true).map_err(failure)?;
            continue;
        }
        file.rewind()?;
        return Ok((file, path));
...
fn pin_id(root: &TypedDigest) -> u128 {
    ...
    h.update(b"rabs.delivery-archive.pin.v1"); h.update(root.bytes);
    let hash = h.finalize();
    let mut bytes = [0;16]; bytes.copy_from_slice(&hash[..16]);
...
fn check_pin(store: &mut dyn RabsMetadataStore, root: &TypedDigest) -> io::Result<bool> {
    match store.pin_row(pin_id(root)).map_err(failure)? {
        None => Ok(false),
        Some(pin) => {
            require(pin.root_key == digest_key(root) && pin.owner == PIN_OWNER && pin.class == PIN_CLASS
                && !pin.released && pin.expires_at_seq.is_none(), "archive pin collision, release or policy mismatch")?;
            Ok(true)
        }
    }
}
...
pub fn archive_delivery(
    cas: &LiveCas, request: &Value, worker: &str, source: &Path, trust: DeliveryTrust,
) -> Result<ArchivedDelivery, String> {
    let result = (|| -> io::Result<ArchivedDelivery> {
        require(!cas.serving_refused, "CAS startup reconciliation refused the store")?;
        ...
        let mut store = cas.store().lock().map_err(|_| invalid("CAS metadata lock poisoned"))?;
        ...
        for (path, item) in &plan {
            ...
            require(matches_item(&digests, item), "delivery changed since verification")?;
            ...
            put_if_absent(cas.layout(), &mut *store, &object, &mut file,
                PutLimits { max_logical_bytes:Some(item.len), expected_size:Some(item.len) },
                DurabilityPolicy::FULL).map_err(failure)?;
        ...
        require(bytes.len() as u64 <= MAX_INDEX_BYTES, "archive index exceeds its budget")?;
        let root = digest_set(&bytes, DigestRequest::default(), None).map_err(failure)?.atp_content_id;
        not_quarantined(&mut *store, &root)?;
        let already_pinned = check_pin(&mut *store, &root)?;
        put_if_absent(cas.layout(), &mut *store, &root, &mut bytes.as_slice(),
            PutLimits { max_logical_bytes:Some(MAX_INDEX_BYTES), expected_size:Some(bytes.len() as u64) },
            DurabilityPolicy::FULL).map_err(failure)?;
        for child in &children {
            store.add_object_edge(&root, child, "delivery-file").map_err(failure)?;
        }
        store.record_manifest(&root, KIND, plan.len() as u64).map_err(failure)?;
        if !already_pinned {
            store.create_pin(pin_id(&root), &root, PIN_OWNER, PIN_CLASS, None, None, true,
                "verified delivery retention; no action publication").map_err(failure)?;
        }
        Ok(ArchivedDelivery { pin:pin_id(&root), root, file_count:plan.len(),
...
fn sync_dirs(root: &Path) -> io::Result<()> {
    for entry in fs::read_dir(root)? {
        let entry = entry?;
        if entry.file_type()?.is_dir() { sync_dirs(&entry.path())?; }
    }
...
pub fn restore_delivery(
    ...
    let result = (|| -> io::Result<Delivery> {
        validate_request(request)?;
        require(!cas.serving_refused, "CAS startup reconciliation refused the store")?;
        require(destination.is_absolute() && destination.components()
            .all(|part| matches!(part, Component::RootDir | Component::Normal(_))), "invalid restore destination")?;
        let root = parse_archive_key(root_key).map_err(io::Error::other)?;
        let mut store = cas.store().lock().map_err(|_| invalid("CAS metadata lock poisoned"))?;
        store.intern_domain(ATP_OBJECT_CONTENT_DOMAIN);
        require(check_pin(&mut *store, &root)?, "archive has no active retention pin")?;
        let (mut file, _) = open_object(&mut *store, &root, MAX_INDEX_BYTES)?;
        let mut bytes = Vec::new();
        let digests = stream(&mut file, &mut bytes, MAX_INDEX_BYTES)?;
        require(digests.atp_content_id == root, "archive index changed while reading")?;
        let index: Value = serde_json::from_slice(&bytes)?;
        require(index.as_object().is_some_and(|map| map.len() == 4)
            && number(&index,"version")? == 1 && text(&index,"kind")? == KIND, "unsupported archive index")?;
        let receipt = &index["receipt"];
        require(text(receipt,"worker_id")? == worker
            && number(receipt,"request_id")? == number(request,"request_id")?
            && text(receipt,"request_sha256")? == hex(&Sha256::digest(serde_json::to_vec(request)?)),
            "archive belongs to a different worker or request")?;
        let plan = items(request, &index["receipt"])?;
        let references = index["objects"].as_array().ok_or_else(|| invalid("archive object list"))?;
        require(references.len() == plan.len(), "archive object set mismatch")?;
        let mut objects = Vec::new();
        for (entry, (path, _)) in references.iter().zip(&plan) {
            let pair = entry.as_array().ok_or_else(|| invalid("archive object entry"))?;
            require(pair.len() == 2 && pair[0].as_str() == Some(path.as_str()), "archive path/order mismatch")?;
        ...
        mkdir(destination)?;
        let staging = destination.join(".restore-staging"); mkdir(&staging)?;
        mkdir(&staging.join("diagnostics"))?; mkdir(&staging.join("artifacts"))?;
        for ((path, item), object) in plan.iter().zip(&objects) {
            ...
            if digests.atp_content_id != *object {
                store.set_location_quarantined(object, &location, true).map_err(failure)?;
                return Err(invalid("CAS replica changed during restore"));
            }
            require(matches_item(&digests,item), "archive object differs from delivery receipt")?;
            #[cfg(unix)] {
                use std::os::unix::fs::PermissionsExt;
                output.set_permissions(fs::Permissions::from_mode(if item.executable {0o700} else {0o600}))?;
            }
            output.sync_all()?;
        }
        let mut marker = create_file(&staging.join("delivery.json"))?;
        marker.write_all(&serde_json::to_vec(&index["receipt"])?)?; marker.sync_all()?;
        drop(marker);
        sync_dirs(&staging)?;
        ...
        fs::rename(staging.join("delivery.json"), destination.join("delivery.pending"))?;
        ...
        fs::rename(destination.join("delivery.pending"),destination.join("delivery.json"))?;
        File::open(destination)?.sync_all()?;
        Ok(Delivery { directory:destination.to_path_buf(), receipt:verified.receipt,
            acknowledgments_confirmed:false,
            acknowledgment_error:Some("restored from CAS; remote acknowledgments were not rechecked".to_owned()) })
    })();
    result.map_err(|error| format!("archive restore refused; no execution or retry was attempted: {error}"))
}
