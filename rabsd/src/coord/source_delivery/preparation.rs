...
//! This selects inputs; it does not discover dependencies or authorize a cache hit.

use super::{
    MAX_SOURCE_ROOTS, SourceUpload, invalid, manifest_value, require, valid_closure_root,
};
use rabs_sandbox::snapshot_capture::{
    MemberDisposition, capture_sealed_source, member_disposition,
};
use rabs_sandbox::source_transfer::{MAX_SOURCE_BYTES, MAX_SOURCE_FILES, SourceFile, SourceManifest};
...
fn local_root(base: &Path, value: &Value) -> io::Result<PathBuf> {
    let path = value.as_str().filter(|path| !path.is_empty() && path.len() <= 4096)
        .ok_or_else(|| invalid("source root path must be a bounded nonempty string"))?;
    if path == "." { return Ok(base.to_path_buf()); }
    require(!path.contains(['\\', ':']) && !path.chars().any(char::is_control)
...
impl SourcePreparation {
    pub(super) fn parse(base: &Path, specification: &Value) -> io::Result<Self> {
        ...
        require(serde_json::to_vec(specification)?.len() <= super::MAX_FRAME_BYTES,
        ...
        if closure {
            require(specification.get("source_files").is_none(),
                "source_roots and source_files are mutually exclusive")?;
            let declared = specification["source_roots"].as_object()
            ...
            for (name, entry) in declared {
                require(valid_closure_root(name) && names.insert(name.to_ascii_lowercase()),
                    "unsafe, hidden or case-colliding source root name")?;
                require(entry.as_object().is_some_and(|fields| fields.len() == 2),
            ...
            selections.push(("workspace".to_owned(), selected_files(&specification["source_files"])?));
        }
        ...
        for (name, files) in &selections {
            for relative in files {
                ...
                {
                    require(member_disposition(path, false) == MemberDisposition::Include,
                        "selected source path is excluded by capture policy")?;
                }
                let path = if closure { format!("{name}/{relative}") } else { relative.clone() };
                require(member_disposition(&path, false) == MemberDisposition::Include,
                    "projected source path is excluded by capture policy")?;
                probe_files.push(SourceFile {
                    path, len:0, sha256:Sha256::digest([]).into(), executable:false,
                });
            }
        }
        let probe_manifest = SourceManifest::new(probe_files)?;
        let mut probe_request = specification.clone();
        let fields = probe_request.as_object_mut().ok_or_else(|| invalid("source preparation object"))?;
        fields.remove("source_files");
        fields.remove("source_roots");
        fields.insert("source_manifest".to_owned(), manifest_value(&probe_manifest));
        super::validate_request(&probe_request)?;
        Ok(Self { roots, selections, closure })
    }

    pub(super) fn root_count(&self) -> usize { self.roots.len() }
    ...
    pub(super) fn capture(&self) -> io::Result<SourceUpload> {
        ...
        let image = Arc::new(capture_sealed_source(&self.roots, false, 2, MAX_SOURCE_BYTES)
        ...
        if self.closure {
            ...
            let (root, files) = self.selections.first()
...
mod tests {
    ...
    fn checkouts(app: &Path, dep: &Path) {
        ...
        fs::write(app.join("Cargo.toml"), b"[dependencies]\ndep = { path = \"../dep\" }\n").unwrap();
        fs::write(dep.join("Cargo.toml"), b"[package]\nname = \"dep\"\nversion = \"0.1.0\"\n").unwrap();
    }
    ...
    fn closure_preparation_preserves_paths_fields_and_replay_without_original_checkouts() {
        ...
        assert_eq!(fs::read(source.join("app/Cargo.toml")).unwrap(), fs::read(app.path().join("Cargo.toml")).unwrap());
        for name in ["app", "dep"] {
        ...
        assert_eq!(request_manifest(&request).unwrap().unwrap().files().len(), 4);
    }
    ...
    fn declaration_validation_precedes_filesystem_access() {
        ...
        let mut bad = Vec::new();
        for (field, value) in [("source_roots", Value::Null), ("source_roots", json!([])),
            ("source_roots", json!({})), ("source_files", json!(["lib.rs"])),
            ("workspace_backing", json!("/ws")), ("source_manifest", Value::Null)] {
            let mut spec = good.clone(); spec[field] = value; bad.push(spec);
        }
        for (field, value) in [("path", json!("../escape")), ("path", json!("x//y")),
            ("path", json!("/tmp/../other")), ("path", json!("/")), ("path", json!(false)),
            ("files", json!([])), ("files", json!(["../escape"])),
            ("files", json!(["src/lib.rs", "src/lib.rs"])),
            ("files", json!(["src", "src/lib.rs"])), ("files", json!(["target/a"])),
            ("files", json!([".git/config"])), ("extra", json!(true))] {
            let mut spec = good.clone(); spec["source_roots"]["dep"][field] = value; bad.push(spec);
        }
        for name in ["target", ".git", "a/b", "..", "app"] {
            let mut spec = good.clone();
            let entry = spec["source_roots"].as_object_mut().unwrap().remove("dep").unwrap();
        ...
        let error = SourcePreparation::parse(base, &spec).unwrap_err().to_string();
    ...
    fn relative_roots_and_root_and_file_count_limits_are_explicit() {
        ...
        assert_eq!(plan.roots, vec![("app".into(), base.to_path_buf()), ("dep".into(), base.join("libs/dep"))]);
        let roots: serde_json::Map<String, Value> = (0..MAX_SOURCE_ROOTS)
            .map(|index| (format!("r{index}"), json!({"path":".", "files":["lib.rs"]}))).collect();
        spec["source_roots"] = Value::Object(roots);
        assert_eq!(SourcePreparation::parse(base, &spec).unwrap().root_count(), MAX_SOURCE_ROOTS);
        spec["source_roots"]["overflow"] = json!({"path":".", "files":["lib.rs"]});
        assert!(SourcePreparation::parse(base, &spec).is_err());
        let files: Vec<_> = (0..MAX_SOURCE_FILES / 2).map(|index| format!("f{index}")).collect();
        spec["source_roots"] = json!({"app":{"path":".","files":files}, "dep":{"path":"libs/dep","files":files}});
        SourcePreparation::parse(base, &spec).unwrap();
        spec["source_roots"]["dep"]["files"].as_array_mut().unwrap().push(json!("one-more"));
        assert!(SourcePreparation::parse(base, &spec).is_err());
    }
    ...
    fn dependency_change_changes_saved_identity_without_replacing_the_old_bundle() {
        ...
        fs::write(dep.path().join("src/lib.rs"), b"pub fn answer() -> u32 { 99 }\n").unwrap();
        ...
        assert_eq!(prepare_source_bundle(app.path(), &spec, &first).unwrap_err().kind(), io::ErrorKind::AlreadyExists);
        assert_eq!(fs::read(first.join("request.json")).unwrap(), old_request);
        assert_eq!(fs::read(first.join("source/dep/src/lib.rs")).unwrap(), b"pub fn answer() -> u32 { 42 }\n");
    }
}
