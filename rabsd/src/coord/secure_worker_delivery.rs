...
pub fn parse_worker_pin(value: &str) -> io::Result<[u8; 32]> {
    require(value.len() == 64 && value.bytes().all(|byte| {
...
fn versions(hello: &Value) -> io::Result<VersionHello> {
    let range = |field: &str| -> io::Result<VersionRange> {
        let read = |key: &str| -> io::Result<u32> {
            hello.get(field).and_then(|value| value.get(key)).and_then(Value::as_u64)
        ...
        Ok(VersionRange { minimum_compatible, current })
    };
    Ok(VersionHello { transport: range("transport")?, application: range("application")? })
}
...
impl<P: WorkerPeer> AdmittedPeer<P> {
    fn new(
        inner: P, identity: TransportIdentity, expected: [u8; 32],
        request: &Value, ids: [u64; 3],
    ) -> io::Result<Self> {
        validate_request(request)?;
        require(serde_json::to_vec(request)?.len() <= MAX_JSON_RECORD, "request exceeds ATP limit")?;
        require(expected != [0; 32] && identity.peer_id == expected && identity.fingerprint == expected,
            "authenticated worker key does not match configured SPKI pin")?;
        require(ids.iter().all(|id| *id != 0), "invalid session challenge identities")?;
        ...
        identities.create(expected, expected, TrustScope::Worker, 1)
            .map_err(|error| invalid(format!("worker identity admission: {error:?}")))?;
        Ok(Self {
            inner, identity, identities, expected_request: request.clone(), ids,
            hello_read: false, negotiation_started: false, proof: None, execution_sent: false,
        })
    }
}
...
impl<P: WorkerPeer> WorkerPeer for AdmittedPeer<P> {
    fn negotiate(&mut self, hello: &Value, grant: &Value) -> io::Result<()> {
        require(self.hello_read && !self.negotiation_started, "session admission cannot be repeated")?;
        self.negotiation_started = true; // a failed exchange never retries admission
        let peer = hex(&self.identity.peer_id);
        require(hello["kind"] == "worker-hello" && hello["peer_id"].as_str() == Some(peer.as_str()),
            "worker hello does not name the authenticated peer")?;
        let slots = hello.get("slots").and_then(Value::as_u64)
            .and_then(|slots| u32::try_from(slots).ok()).filter(|slots| *slots > 0)
            .ok_or_else(|| invalid("invalid worker slots"))?;
        require(grant["kind"] == "session-ok" && grant["output_transfer"] == "ranges-v1"
            && grant["recovery_protocol"] == "request-journal-v1", "invalid delivery session selection")?;
        ...
        let token = mint(token_id, CapabilityKind::ExecuteAction, session_id, operation_id, &scope, 2)
            .map_err(|error| invalid(format!("session token: {error:?}")))?;
        let claims = WorkerHelloClaims {
            versions: versions(hello)?, claimed_peer: self.identity.peer_id,
            session_id, operation_id, token: token.clone(),
            canonical_namespace: hello.get("canonical").and_then(Value::as_bool) == Some(true), slots,
        };
        let ours = VersionHello {
            transport: VersionRange { minimum_compatible: 1, current: 1 },
            application: VersionRange { minimum_compatible: 1, current: 1 },
        };
        let admitted = CoordinatorSession::admit_hello(&self.identities, &self.identity, &claims,
            &SessionPolicy {
                ours, revoked_token_ids: &[], current_seq: 1,
                required_capability: CapabilityKind::ExecuteAction, require_canonical: true,
                limits: DEFAULT_LIMITS, max_sequence_buffer: 1,
            }).map_err(|error| invalid(format!("worker session refused: {error:?}")))?;
        require(admitted.grant().scope == TrustScope::Worker, "identity lacks worker role")?;
        ...
        require(response["kind"] == "worker-auth"
            ...
            && response["token_id"].as_u64() == Some(token_id), "worker challenge response mismatch")?;
        ...
        self.proof = Some(WorkerAuthentication {
            spki_sha256: self.identity.fingerprint, session_id,
    ...
    fn send(&mut self, frame: &Value) -> io::Result<()> {
        require(self.proof.is_some(), "execution/delivery before authenticated admission")?;
        match frame["kind"].as_str() {
            Some("canonical-exec") => {
                require(!self.execution_sent && frame == &self.expected_request,
            ...
            Some("output-read" | "artifact-read" | "output-ack" | "artifact-ack") => {
                require(self.execution_sent && frame["request_id"] == self.expected_request["request_id"],
                    "delivery request does not own this execution")?;
            }
            _ => return Err(invalid("unsupported coordinator operation on delivery session")),
        }
    ...
    fn receive(&mut self) -> io::Result<Value> {
        if self.proof.is_none() {
            ...
            require(hello["kind"] == "worker-hello", "expected authenticated worker hello")?;
            return Ok(hello);
        }
        require(self.execution_sent, "unsolicited worker result")?;
        let frame = self.inner.receive()?;
        require(matches!(frame["kind"].as_str(), Some(
            "exec-result" | "heartbeat" | "error" | "output-chunk" | "artifact-chunk"
            | "output-acknowledged" | "artifact-acknowledged"
...
enum Phase { Admission, Execution, Transfer, Acknowledgment }
...
impl<'a, S: AsyncRead + AsyncWrite + Unpin> RecordPeer<'a, S> {
    fn new(runtime: &'a Runtime, stream: S, request: &Value) -> Self {
        let millis = request.get("timeout_ms").and_then(Value::as_u64)
            .unwrap_or(MAX_EXECUTION_MS).min(MAX_EXECUTION_MS);
        Self {
            runtime, stream, until: Instant::now() + ADMISSION_BUDGET,
            execution_budget: Duration::from_millis(millis) + Duration::from_secs(60),
            phase: Phase::Admission, failed: false,
        }
    }

    fn remaining(&self) -> io::Result<Duration> {
        if self.failed { return Err(invalid("worker connection failed; no retry")); }
        self.until.checked_duration_since(Instant::now()).filter(|left| !left.is_zero())
            .ok_or_else(|| io::Error::new(io::ErrorKind::TimedOut, "worker phase deadline exceeded"))
    }
}
...
impl<S: AsyncRead + AsyncWrite + Unpin> WorkerPeer for RecordPeer<'_, S> {
    fn send(&mut self, frame: &Value) -> io::Result<()> {
        ...
        match frame["kind"].as_str() {
            Some("canonical-exec") => {
                require(self.phase == Phase::Admission, "duplicate execution dispatch")?;
    ...
    fn receive(&mut self) -> io::Result<Value> {
        ...
        let result = self.runtime.block_on(async {
            asupersync::time::timeout(asupersync::time::wall_now(), budget, async {
                ...
                loop {
                    ...
                    if byte[0] == b'\n' {
                        return serde_json::from_slice::<Value>(&bytes).map_err(|error| invalid(error.to_string()));
                    }
                    require(bytes.len() < MAX_JSON_RECORD, "worker record exceeds ATP limit")?;
                    bytes.push(byte[0]);
                }
        ...
        if let Ok(value) = &result
            && self.phase == Phase::Execution && value["kind"] == "exec-result"
        {
...
pub fn receive_authenticated(
    runtime: &Runtime, peer: AuthenticatedPeer, expected_spki: [u8; 32],
    expected_worker: &str, request: &Value, destination: &Path,
) -> Result<Delivery, DeliveryFailure> {
    let failure = |error: io::Error| DeliveryFailure {
        directory: destination.to_path_buf(), execution_may_have_run: false, detail: error.to_string(),
    };
    if asupersync::cx::Cx::current().is_some() {
        return Err(failure(invalid("authenticated delivery requires an operator thread, not nested block_on")));
    }
    let raw = RecordPeer::new(runtime, peer.stream, request);
    let mut admitted = AdmittedPeer::new(raw, peer.identity, expected_spki, request,
        challenge_ids().map_err(&failure)?).map_err(failure)?;
...
mod tests {
    ...
    impl WorkerPeer for Script {
        fn send(&mut self, frame: &Value) -> io::Result<()> {
            ...
            if self.fail_execution && frame["kind"] == "canonical-exec" {
                return Err(io::Error::new(io::ErrorKind::BrokenPipe, "partial execution write"));
            }
            Ok(())
        }
        fn receive(&mut self) -> io::Result<Value> {
            self.replies.pop_front().ok_or_else(|| invalid("missing scripted response"))
        }
    }
    fn request() -> Value {
    ...
    fn peer(hello: Value, response: Value) -> AdmittedPeer<Script> {
        let inner = Script { replies: VecDeque::from([hello, response]), ..Script::default() };
        AdmittedPeer::new(inner, TransportIdentity { peer_id:[1; 32], fingerprint:[1; 32] },
            [1; 32], &request(), [10, 20, 30]).unwrap()
    }

    #[test]
    fn pin_is_explicit_and_identity_must_be_transport_proven() {
        assert_eq!(parse_worker_pin(&"01".repeat(32)).unwrap(), [1; 32]);
        for value in ["00".repeat(32), "AA".repeat(32), "01".repeat(31), "xyz".to_owned()] {
            assert!(parse_worker_pin(&value).is_err());
        }
        for identity in [
            TransportIdentity { peer_id:[2; 32], fingerprint:[1; 32] },
            TransportIdentity { peer_id:[1; 32], fingerprint:[2; 32] },
        ] {
            assert!(AdmittedPeer::new(Script::default(), identity, [1; 32], &request(), [10,20,30]).is_err());
        }
    ...
    fn malformed_or_incompatible_versions_refuse_before_challenge() {
        for range in [
            ...
            json!({"minimum_compatible":0,"current":1}), Value::Null,
        ] {
            let mut hello = hello();
    ...
    fn challenge_binds_session_operation_token_and_peer_before_any_execution() {
        for field in ["session_id", "operation_id", "token_id", "peer_id", "kind"] {
            ...
            assert!(peer.send(&request()).is_err());
            assert!(peer.negotiate(&hello, &grant()).is_err(), "no handshake retry");
            assert!(peer.authentication().is_none());
        }
    }
    ...
    fn successful_admission_preserves_transfer_selection_and_limits_dispatch() {
        ...
        assert!(peer.send(&json!({"kind":"artifact-read","request_id":8})).is_err());
        assert_eq!(peer.inner.sent.iter().filter(|frame| frame["kind"] == "canonical-exec").count(), 1);
        peer.inner.replies.push_back(json!({"kind":"commit","request_id":7}));
