...
fn immutable_snapshot_source_refuses_writes_and_still_builds() {
    ...
    // The lockfile MUST be part of the immutable snapshot (it is in
    // D018's always-include set for exactly this reason): cargo cannot
    // create one inside a read-only source, by design.
    write(
        source.path(),
        "Cargo.lock",
