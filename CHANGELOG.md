...
- **Per-project worker routing via `[routing] preferred_workers` in `.rch/config.toml`.**
  The daemon's selector has honored `SelectionRequest.preferred_workers` as a hard
  preference since v1.0.41 (selects only listed workers when any is eligible; falls
  back to all eligible only if none are), but the hook only ever populated it from the
  `RCH_WORKER`/`RCH_WORKERS` env. The hook now *also* reads a `[routing] preferred_workers`
  list from the project-local `.rch/config.toml` (relative to the build cwd) and merges it
  with the env list. This lets a repo durably pin its remote builds to specific workers —
  e.g. routing a project with a huge target dir to big-disk workers so it never wedges a
  small-disk one. Best-effort: a missing/malformed project config yields no pinning, never
  an error, so it can't break the build path.

> Branched from v1.0.41 (the deployed fleet baseline); does not include the v1.0.42 tag's
> changes or the unreleased `main` work-in-progress.
