...
emit setup begin INFO "log=$LOG_FILE tmp=$TMP rch_wkr=$RCHWKR"

# Build rch-wkr release if missing.
if [ ! -x "$RCHWKR" ]; then
    emit setup build_rch_wkr INFO
    (cd "$PROJECT_ROOT" && cargo build --release -p rch-wkr) >>"$LOG_FILE" 2>&1
fi
...
"$RCHWKR" prepare --project "$TMP/rust_proj" --runtime rust >"$TMP/rust_prep.json" 2>>"$LOG_FILE"
...
"$RCHWKR" prepare --project "$TMP/bad_proj" --runtime bun >"$TMP/bad_prep.json" 2>>"$LOG_FILE" || true
...
"$RCHWKR" prepare --project "$PROJ" --runtime bun >"$TMP/prep1.json" 2>>"$LOG_FILE"
...
"$RCHWKR" prepare --project "$PROJ" --runtime bun >"$TMP/prep2.json" 2>>"$LOG_FILE"
ACTION2=$(jq -r '.action' "$TMP/prep2.json")
TOOK2=$(jq -r '.took_ms' "$TMP/prep2.json")
if [ "$ACTION2" = "Skipped" ] && [ "$TOOK2" -lt "$TOOK1" ]; then
    PASS=$((PASS + 1))
    emit s2 cache_hit PASS "took_ms=$TOOK2 vs first=$TOOK1"
else
    FAIL=$((FAIL + 1))
    emit s2 cache_hit FAIL "action=$ACTION2 took=$TOOK2 first=$TOOK1"
fi
...
python3 -c "
import json, sys
with open('$PROJ/package.json') as f: d = json.load(f)
d['description'] = 'Updated via e2e test'
with open('$PROJ/package.json', 'w') as f: json.dump(d, f, indent=2)
"
"$RCHWKR" prepare --project "$PROJ" --runtime bun >"$TMP/prep3.json" 2>>"$LOG_FILE"
