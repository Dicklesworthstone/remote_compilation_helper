...
mod tests {
    ...
    /// A `version = 4` lockfile consistent with an `app` binary crate that
    /// path-depends on `dep_lib` (no registry packages, so it is fully valid
    /// offline).
    #[cfg(unix)]
    fn valid_app_lockfile() -> &'static str {
    ...
    /// A lockfile that no longer matches the manifests: it predates the
    /// `dep_lib` path dependency, so `cargo metadata --locked` must refuse
    /// to use it (and must not rewrite it).
    #[cfg(unix)]
    fn stale_app_lockfile() -> &'static str {
        "# This file is automatically @generated by Cargo.\n\
         # It is not intended for manual editing.\n\
         version = 4\n\n\
         [[package]]\n\
    ...
    impl LockPlanningFixture {
        fn new(prefix: &str) -> Self {
            ...
            let dep_root = scenario_root.join("dep_lib");
            write_bin_crate(&app_root, "app", &[("dep_lib", "../dep_lib")]);
            write_lib_crate(&dep_root, "dep_lib", &[]);
        ...
        fn lock_path(&self) -> PathBuf {
            self.app_root.join("Cargo.lock")
        }

        fn resolve_package_roots(&self) -> BTreeSet<PathBuf> {
            let graph =
                resolve_cargo_path_dependency_graph_with_policy(&self.app_root, &self.policy)
                    .expect("resolve app graph");
            graph
                .packages
                .iter()
        ...
        fn assert_discovers_dep(&self) {
            let roots = self.resolve_package_roots();
            let dep_root = self.dep_root.canonicalize().expect("canonical dep root");
            assert!(
                roots.contains(&dep_root),
                "path dependency root must be discovered: {roots:?}"
            );
        }
    }
    ...
    fn valid_lockfile_planning_discovers_roots_without_mutating_the_lock() {
        ...
        fs::write(&lock_path, valid_app_lockfile()).expect("write valid lock");
        let bytes_before = fs::read(&lock_path).expect("read lock before");
        let mtime_before = fs::metadata(&lock_path)
            .and_then(|meta| meta.modified())
            .expect("lock mtime before");

        fixture.assert_discovers_dep();

        let bytes_after = fs::read(&lock_path).expect("read lock after");
        let mtime_after = fs::metadata(&lock_path)
            .and_then(|meta| meta.modified())
            .expect("lock mtime after");
        assert_eq!(
            bytes_before, bytes_after,
            "valid lockfile bytes must be preserved"
        );
        assert_eq!(
            mtime_before, mtime_after,
            "valid lockfile mtime must be preserved"
        );
    }
    ...
    fn stale_lockfile_planning_falls_back_without_rewriting_the_lock() {
        let fixture = LockPlanningFixture::new("lock-stale");
        let lock_path = fixture.lock_path();
        fs::write(&lock_path, stale_app_lockfile()).expect("write stale lock");
        let bytes_before = fs::read(&lock_path).expect("read lock before");
        let mtime_before = fs::metadata(&lock_path)
            .and_then(|meta| meta.modified())
            .expect("lock mtime before");
        ...
        assert_eq!(
            bytes_before, bytes_after,
            "stale lockfile bytes must be preserved, never regenerated"
        );
        assert_eq!(
            mtime_before, mtime_after,
            "stale lockfile mtime must be preserved"
        );
    }
