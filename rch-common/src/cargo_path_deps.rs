...
//! 1. `cargo metadata --locked` (primary source of truth when a valid
//!    `Cargo.lock` is present)
//! 2. Recursive manifest parsing fallback (for malformed metadata, metadata
//!    failures, and projects whose lockfile is stale, incompatible, or absent)
//!
//! Dependency planning is strictly NON-MUTATING
//! (bd-rch-pinned-metadata-lock-ytkda): plain `cargo metadata` creates or
//! rewrites the project's `Cargo.lock` as a side effect, which silently
//! mutated user checkouts every time the hook planned a sync closure. The
//! resolver therefore always passes `--locked`; when cargo refuses (stale or
//! missing lockfile) it falls through to the manifest-only fallback, which
//! never writes anything. Lockfile bytes and mtimes are preserved in every
//! case.
//!
//! This invariant is about the ambient checkout passed to dependency planning.
//! Clean-overlay execution materializes a pinned Git archive into a private
//! remote root and deliberately does not run this ambient dependency planner.
//! Mutation inside that private archive would be a separate defect and is not
//! evidence that the ambient checkout changed; conversely, archive-only checks
//! cannot prove this checkout-level invariant.
//!
...
fn invoke_cargo_metadata(manifest_path: &Path) -> Result<String, CargoPathDependencyError> {
    ...
    // `--locked` is load-bearing (bd-rch-pinned-metadata-lock-ytkda): without
    // it, `cargo metadata` creates or rewrites the project's `Cargo.lock` as
    // a side effect, mutating the very checkout RCH is only supposed to
    // read. With `--locked`, a valid lockfile succeeds byte-for-byte
    // untouched, while a stale or missing lockfile makes cargo REFUSE
    // without writing — and that refusal flows into the manifest-only
    // fallback, which is non-mutating by construction.
...
mod tests {
    ...
    #[cfg(unix)]
    struct LockPlanningFixture {
        _fixture: TopologyFixture,
        workspace_root: PathBuf,
        app_root: PathBuf,
        late_dep_root: PathBuf,
        cargo_home: PathBuf,
    ...
    #[cfg(unix)]
    impl LockPlanningFixture {
        fn new(prefix: &str) -> Self {
            let fixture = TopologyFixture::new(prefix);
            let scenario_root = fixture.canonical_root.join(prefix);
            let workspace_root = scenario_root.join("ambient_checkout");
            let app_root = workspace_root.join("app");
            let normal_dep_root = scenario_root.join("normal_dep");
            ...
            let patch_only_dep_root = scenario_root.join("patch_only_dep");
            let late_dep_root = scenario_root.join("late_dep");
            let cargo_home = scenario_root.join("cargo_home");

            write_lib_crate(&normal_dep_root, "normal_dep", &[]);
            ...
            write_lib_crate(&patch_only_dep_root, "patch_only_dep", &[]);
            fs::create_dir_all(app_root.join("src")).expect("create app source root");
            fs::write(app_root.join("src/main.rs"), "fn main() {}\n").expect("write app main");
            Self::write_app_manifest(&app_root, false);
            fs::write(
...
[patch.crates-io]
patch_only_dep = { path = "../patch_only_dep" }
"#,
            )
        ...
        fn write_app_manifest(app_root: &Path, include_late_dependency: bool) {
            let late_dependency = if include_late_dependency {
                "late_dep = { path = \"../../late_dep\" }\n"
            } else {
                ""
            };
            fs::write(
...
"#
        ...
        fn generate_valid_lockfile(&self) {
            let output = Command::new("cargo")
                ...
                .output()
                .expect("run real cargo generate-lockfile");
            assert!(
                output.status.success(),
                "cargo generate-lockfile failed: {}",
                String::from_utf8_lossy(&output.stderr)
            );
            assert!(self.lock_path().is_file(), "Cargo generated the lockfile");
        }

        fn make_generated_lock_stale(&mut self) {
            write_lib_crate(&self.late_dep_root, "late_dep", &[]);
            self.expected_roots.insert(
                self.late_dep_root
                    .canonicalize()
                    .expect("canonical late dependency"),
            );
            Self::write_app_manifest(&self.app_root, true);
        }

        fn resolve_planned_roots(&self) -> BTreeSet<PathBuf> {
            let plan = crate::build_dependency_closure_plan_with_policy(
                &self.workspace_root,
                &self.policy,
            );
            assert!(
                plan.is_ready(),
                "dependency plan must be ready; reason={:?}, issues={:?}",
        ...
        fn assert_discovers_every_path_root(&self) {
            assert_eq!(
                self.resolve_planned_roots(),
                self.expected_roots,
                "locked-metadata success or manifest fallback must preserve the complete path-root closure"
            );
        }
    }
    ...
    impl LockfileSnapshot {
        fn capture(path: &Path) -> Self {
            Self {
                bytes: fs::read(path).expect("read lockfile snapshot"),
                modified: fs::metadata(path)
                    .and_then(|metadata| metadata.modified())
                    .expect("read lockfile mtime"),
            }
        }

        fn arm_mtime_detection() {
            // Keep the pre-plan mtime outside common one-second timestamp
            // granularity. A byte-identical rewrite must still fail the mtime
            // assertion below instead of disappearing inside one clock tick.
            std::thread::sleep(std::time::Duration::from_millis(1100));
        }

        fn assert_unchanged(&self, path: &Path, scenario: &str) {
            assert_eq!(
                fs::read(path).expect("read lockfile after planning"),
                self.bytes,
                "{scenario}: dependency planning changed Cargo.lock bytes"
            );
            assert_eq!(
                fs::metadata(path)
                    .and_then(|metadata| metadata.modified())
                    .expect("read lockfile mtime after planning"),
                self.modified,
                "{scenario}: dependency planning changed Cargo.lock mtime"
            );
        }
    }

    /// bd-rch-pinned-metadata-lock-ytkda: with a VALID lockfile, dependency
    /// planning uses `cargo metadata --locked` and leaves the lockfile
    /// byte-for-byte and mtime-for-mtime untouched.
    #[cfg(unix)]
    #[test]
    fn valid_lockfile_planning_discovers_roots_without_mutating_the_lock() {
        let fixture = LockPlanningFixture::new("lock-valid");
        let lock_path = fixture.lock_path();
        fixture.generate_valid_lockfile();
        let snapshot = LockfileSnapshot::capture(&lock_path);
        LockfileSnapshot::arm_mtime_detection();

        invoke_cargo_metadata(&fixture.manifest_path())
            .expect("real cargo metadata --locked accepts the generated valid lock");
        fixture.assert_discovers_every_path_root();

        snapshot.assert_unchanged(&lock_path, "valid lockfile");
    }

    /// bd-rch-pinned-metadata-lock-ytkda: a STALE lockfile makes
    /// `cargo metadata --locked` refuse; planning falls back to manifest-only
    /// discovery, still finds the path roots, and never rewrites the stale
    /// lock (bytes and mtime preserved).
    #[cfg(unix)]
    #[test]
    fn stale_lockfile_planning_falls_back_without_rewriting_the_lock() {
        let mut fixture = LockPlanningFixture::new("lock-stale");
        fixture.generate_valid_lockfile();
        let lock_path = fixture.lock_path();
        fixture.make_generated_lock_stale();
        let snapshot = LockfileSnapshot::capture(&lock_path);
        LockfileSnapshot::arm_mtime_detection();

        let refusal = invoke_cargo_metadata(&fixture.manifest_path())
            .expect_err("real cargo metadata --locked must refuse a generated stale lock");
        assert_eq!(
            refusal.kind(),
            &CargoPathDependencyErrorKind::MetadataInvocationFailure
        );
        snapshot.assert_unchanged(&lock_path, "locked metadata refusal");

        fixture.assert_discovers_every_path_root();
        snapshot.assert_unchanged(&lock_path, "stale-lock fallback planning");
    }

    /// bd-rch-pinned-metadata-lock-ytkda: with NO lockfile, planning stays
    /// discoverable through the manifest-only fallback and must not create a
    /// `Cargo.lock` as a side effect (the historical mutation bug).
    #[cfg(unix)]
    #[test]
    fn missing_lockfile_planning_discovers_roots_without_creating_a_lock() {
        let fixture = LockPlanningFixture::new("lock-missing");
        let lock_path = fixture.lock_path();
        assert!(!lock_path.exists(), "fixture starts without a lockfile");

        let refusal = invoke_cargo_metadata(&fixture.manifest_path())
            .expect_err("real cargo metadata --locked must refuse an absent lock");
        assert_eq!(
            refusal.kind(),
            &CargoPathDependencyErrorKind::MetadataInvocationFailure
        );
        assert!(
            !lock_path.exists(),
            "locked metadata refusal must not create Cargo.lock"
        );
        ...
        assert!(
            !lock_path.exists(),
            "dependency planning must never create a Cargo.lock in the user checkout"
        );
    }
