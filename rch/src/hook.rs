...
// `progress_reporting` submodule. `BuildHeartbeatLoop` and
// `mark_heartbeat_progress` are imported so `execute_remote_compilation` calls
// them unqualified.
...
// adapter invocation + contract/auth resolution) lives in the `repo_updater`
// submodule. `maybe_sync_repo_set_with_repo_updater` is its only cross-module
// entry point (called from `execute_remote_compilation`); the remaining imports
// are the helpers the hook test suite exercises directly.
mod repo_updater;
use repo_updater::maybe_sync_repo_set_with_repo_updater;
...
mod tests {
    use super::*;
    // repo_updater helpers exercised directly by the hook test suite (the
    // submodule keeps them `pub(super)`; they are test-only so they are imported
