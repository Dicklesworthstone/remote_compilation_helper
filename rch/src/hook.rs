...
// `ssh` submodule. `run_offload_ssh_command` / `should_skip_remote_preflight`
// are imported here for `verify_remote_dependency_manifests`; the sibling
// submodules (`transfer_orchestration`, `repo_updater`) import what they need
// directly from `super::ssh`.
mod ssh;
use ssh::{run_offload_ssh_command, should_skip_remote_preflight};
