...
// `ssh` submodule. Sibling submodules (`dependency_closure`,
// `transfer_orchestration`, `repo_updater`) import what they need directly
// from `super::ssh`, and the doctor's mirror-ownership probe consumes the
// same items via `crate::hook::ssh`, so the module stays crate-internal
// (`pub(crate)`) rather than fully private.
pub(crate) mod ssh;
