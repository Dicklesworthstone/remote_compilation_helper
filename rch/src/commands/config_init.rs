...
use rch_common::{ApiResponse, TransferConfig};
...
/// Format the default exclude patterns from `TransferConfig::default()` as TOML array entries.
fn format_default_exclude_patterns() -> String {
    TransferConfig::default()
        .exclude_patterns
        .iter()
        .map(|p| format!("    {},\n", toml_string_literal(p)))
        .collect::<String>()
}
...
fn config_init_simple(
    ...
    if !config_path.exists() {
        let exclude_lines = format_default_exclude_patterns();
...
fn generate_config_toml(values: &ConfigValues) -> String {
    let exclude_lines = format_default_exclude_patterns();
    let log_level = toml_string_literal(&values.log_level);
    let socket_path = toml_string_literal(&values.socket_path);
    format!(
        r###"# RCH Configuration
...
fn generate_workers_toml(workers: &[WizardWorker]) -> Result<String> {
    ...
    let rendered =
        toml::to_string_pretty(&config).context("Failed to serialize wizard workers.toml")?;
...
mod tests {
    ...
    fn generate_workers_toml_single_worker() {
        ...
        let toml = generate_workers_toml(&workers).expect("workers TOML should serialize");
    ...
    #[test]
    fn generate_config_toml_escapes_string_values() {
        let _guard = test_guard!();
        let values = ConfigValues {
            log_level: "info\"debug".to_string(),
            socket_path: "/tmp/rch.sock\"\n[evil]\nkey = \"value".to_string(),
            confidence_threshold: 0.85,
            min_local_time_ms: 2000,
            compression_level: 3,
        ...
        let toml = generate_config_toml(&values);
        let parsed: toml::Value = toml::from_str(&toml).expect("config TOML should parse");

        assert_eq!(
            parsed
                .get("general")
                .and_then(|general| general.get("log_level"))
        ...
        assert_eq!(
            parsed
                .get("general")
                .and_then(|general| general.get("socket_path"))
                .and_then(toml::Value::as_str),
            Some("/tmp/rch.sock\"\n[evil]\nkey = \"value")
        );
    ...
    #[test]
    fn generate_workers_toml_escapes_string_values() {
        let _guard = test_guard!();
        let workers = vec![WizardWorker {
            id: "worker-1".to_string(),
            host: "host\"\n[[workers]]\nid = \"injected".to_string(),
            user: "ubuntu\"admin".to_string(),
            identity_file: "/tmp/key\\with\"quote".to_string(),
            total_slots: 8,
            priority: 100,
        }];

        let toml = generate_workers_toml(&workers).expect("workers TOML should serialize");
        let parsed: WorkersConfig = toml::from_str(&toml).expect("workers TOML should parse");

        assert_eq!(parsed.workers.len(), 1);
        assert_eq!(parsed.workers[0].id, "worker-1");
        assert_eq!(
            parsed.workers[0].host,
            "host\"\n[[workers]]\nid = \"injected"
        );
        assert_eq!(parsed.workers[0].user, "ubuntu\"admin");
        assert_eq!(parsed.workers[0].identity_file, "/tmp/key\\with\"quote");
    }
