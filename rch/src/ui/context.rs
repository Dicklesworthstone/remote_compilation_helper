...
mod tests {
    ...
    fn test_machine_mode_toon_output() {
        ...
        let output = stdout_buf.to_string_lossy();
        let output_json = toon_rust::toon_to_json(&output).unwrap();
        let actual_json: serde_json::Value = serde_json::from_str(&output_json).unwrap();
        let expected = serde_json::to_value(&value).unwrap();
        assert!(
            json_values_loose_eq(&actual_json, &expected),
            "TOON output did not match expected JSON"
        );
    }
